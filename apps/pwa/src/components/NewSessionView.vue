...
import type { Repo } from '@flux/protocol';
import { computed, onMounted, ref } from 'vue';

import type { Store } from '../store/create-store.ts';

// Start an agent: pick a repo, name the branch (a new one is created from the repo's HEAD, an
// existing one is checked out), write the first prompt.
...
const ready = computed(
  () => repo.value !== '' && branch.value.trim() !== '' && prompt.value.trim() !== '',
);
