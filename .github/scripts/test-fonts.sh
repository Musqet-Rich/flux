...
sed "s/'Family code 3',/'Family code 3', 'Family code 10',/" "$tmp/fonts.ts" | sed "s/'Family code 3', 'Family code 10',/'Family code 10',/" >"$tmp/ts2"
...
sed 's/family-code-4-var/family-code-4-bold/' "$tmp/fonts.css" >"$tmp/css2"
...
# An @font-face the list does not name is fine: the landing's own families are declared too.
{ cat "$tmp/fonts.css"; echo '@font-face {'; echo "  font-family: 'Landing';"; echo "  src: url('/fonts/family-text-0-var.woff2') format('woff2');"; echo '}'; } >"$tmp/css3"
run ok  'a family declared but not offered'           sh "$check" "$tmp/fonts.ts" "$tmp/css3" "$tmp/base.css" "$tmp/fonts"
