...
run bad 'no face cut to the Latin block'              sh "$check" "$tmp/fonts.ts" "$tmp/css6" "$tmp/base.css" "$tmp/fonts"
awk 'BEGIN { n = 0 } /unicode-range/ { n++; if (n == 7) { print "  unicode-range: U+0041;"; next } } { print }' "$tmp/fonts.css" >"$tmp/css7"
run bad 'one face trimmed to less than the others'    sh "$check" "$tmp/fonts.ts" "$tmp/css7" "$tmp/base.css" "$tmp/fonts"
awk 'BEGIN { n = 0 } /font-weight/ { n++; if (n == 3) next } { print }' "$tmp/fonts.css" >"$tmp/css8"
run bad 'a face with no weight'                       sh "$check" "$tmp/fonts.ts" "$tmp/css8" "$tmp/base.css" "$tmp/fonts"
awk 'BEGIN { n = 0 } /src:/ { n++; if (n == 3) next } { print }' "$tmp/fonts.css" >"$tmp/css9"
run bad 'a face with no file'                         sh "$check" "$tmp/fonts.ts" "$tmp/css9" "$tmp/base.css" "$tmp/fonts"
# The real stylesheet's range is wrapped over lines; the check reads it whole.
awk '/unicode-range/ { print "  unicode-range:"; print "    U+0000-00FF;"; next } { print }' "$tmp/fonts.css" >"$tmp/css10"
run ok  'a range wrapped over lines'                  sh "$check" "$tmp/fonts.ts" "$tmp/css10" "$tmp/base.css" "$tmp/fonts"
