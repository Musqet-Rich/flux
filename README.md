...
The dev server proxies `/ws` and `/healthz` to the relay (`FLUX_DEV_RELAY` overrides `http://127.0.0.1:8787`), so the app reaches the box through its own origin just like the built one. To pair, take the link `flux pair` (or the daemon's start-up QR) prints, which has origin `http://127.0.0.1:8787`, and open `http://localhost:5173/#<fragment>` with the same fragment; the app pairs from the fragment on load. Edits to components then hot-reload without dropping the paired connection. The service worker is not registered under the dev server, so there are no push notifications there.
