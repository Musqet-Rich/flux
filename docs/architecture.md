...
- `git/`: `conventionalCommit` (`matches` a `type(scope)?!?: subject` line of at most 100 characters; `latest` picks the commit that seeds the PR title from a `git.log` page). `git.log` is the branch's history with the base included: the device cannot tell where the branch forked, so a fresh branch seeds the title from the base's last commit until the agent commits.
