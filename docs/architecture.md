...
8. **Lifecycle** (ADR 0017). `flux daemon` takes `<dataDir>/daemon.lock` (its pid; exit 3 if another live daemon holds it, a stale one is replaced), settles what the last daemon left (`ask` without `ask.answered` in a `running` or `waiting_user` session → `ask.answered aborted`; the session → `idle`, reason `daemon restarted`), then binds the control socket and connects. `stop()` is bounded: transport off, pending asks aborted, control connections destroyed, each agent closed by stdin EOF → SIGTERM → SIGKILL of its process group with a grace per stage, sessions caught mid-turn logged `idle`, stores closed, lock released. A second signal or a 10 s budget exits at once.
