...
6. **The user triggers a daemon update from the PWA, targeting the version the box itself discovered and verified.** The box, not the PWA, decides what "latest" is: the PWA calls `daemon.checkUpdate`, and the daemon discovers the newest published release (`releases/latest`) and runs a verify-only dry-run against the trusted keys before reporting it (ADR 0022). When the box reports a newer, verified release than the running `hello.version`, the app offers "Update daemon". Accepting sends the `daemon.update` RPC with **that box-discovered, box-verified version** as the target — not the PWA's own build version — so the box installs exactly the release its own dry-run already proved. The daemon fetches that release, verifies it again and restarts into it (ADR 0022). The PWA shows progress from the daemon's events and reconnects when the new version answers `hello`. The daemon is never updated from the wire without the operator asking.
