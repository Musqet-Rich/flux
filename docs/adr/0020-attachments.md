...
5. **Lifetime.** Cleanup is lazy, with no timer: at daemon start and before each `attach.begin`, an upload not ended within 10 minutes and a complete attachment not sent within 24 hours are deleted. A channel that goes away mid-upload is covered by the 10 minutes. `sessions.archive` removes the session's whole attachments directory (and rows); the log keeps the names and sizes, so the timeline still lists what was sent, without thumbnails.
...
- The box holds up to 50 MiB per message on disk, under the data directory, until the session is archived. Nothing counts against the worktree or the repository.
