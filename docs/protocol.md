...
| `sessions.create`    | `{ repo; branch; base?: string; harness; title?; model?: string; effort?: string }`  | `SessionSummary` (`harness` is the runtime `claude`/`pi`; `model`/`effort` are loose non-empty strings the box compiles to harness flags, ADR 0023, omitted to spawn on the box defaults)                                                                                                                                                                                                                                                                                                                                                                             |
...
| `settings.set`       | `{ flux?: Partial<FluxSettings>; harnessConfig?: Partial<HarnessConfig> }`           | `Settings` (the whole state after the patch; `bad_params` and nothing written if any part is invalid, including an unknown key; `agent_unavailable` when `flux.defaultHarness` names a harness the box did not find, see `hello.agents`)                                                                                                                                                                                                                                                                                                                              |
