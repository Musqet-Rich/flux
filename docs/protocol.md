...
type FluxEvent =
  ...
  | Envelope<'session.head', { head: string }> // the worktree's HEAD moved, to another branch (an agent's `git switch`) or off any (detached, given as its short sha); checked when a turn ends, at a `vcs.changed` notice and before the message that spawns an agent, so `SessionSummary.head` follows
  | Envelope<'session.cleared', Record<string, never>> // the agent's context was dropped (sessions.clear, or a bare `/clear` sent as a message); what follows is a fresh conversation, and where a device's timeline opens
...
type Ephemeral =
  ...
  | { type: 'update.progress'; phase: 'fetching' | 'verifying' | 'installing' | 'restarting' } // self-update phase (ADR 0022)
  | {
      type: 'update.failed';
      reason:
        'bad_signature' | 'download_failed' | 'already_current' | 'unsupported' | 'disk_error';
    } // self-update aborted
  | { type: 'shell.output'; runId: string; stream: 'stdout' | 'stderr'; chunk: string } // command-runner output (ADR 0026)
  | {
      type: 'shell.exited';
      runId: string;
      code: number | null;
...
`update.progress` and `update.failed` are the other two session-less ephemerals, emitted by the daemon during a self-update (`daemon.update`, § 7, ADR 0022). `update.progress` names the phase the daemon has reached (`fetching` → `verifying` → `installing` → `restarting`); `update.failed` names the reason it aborted, the running daemon untouched. Like every ephemeral they are never logged and may be dropped. Success has **no** event: the daemon exits after the swap, the channel drops, and the device reconnects and reads the new `hello.version`, which is how it learns the update landed.

`shell.output` and `shell.exited` are the command-runner's session-less ephemerals (ADR 0026, § 7 `shell.*`). While an operator-started one-off command runs, its stdout and stderr stream as `shell.output` chunks (coalesced by the daemon before broadcast), and `shell.exited` ends it with the process's `code`/`signal` (one is null, as usual for a child exit) and `truncated` (true when the daemon killed the process on the 256 KiB output cap or the 10-minute wall-clock). `runId` ties them to the `shell.run` that started the run. Like every ephemeral they are **never logged** — command output touches neither the event log nor the database — and may be dropped; on reconnect in-flight output is simply missed. They carry no `session`: a run is not a session.
...
| --------------------- | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hello`               | `{ protocol: 2; client: string }`                                                                   | `{ protocol: 2; daemon: string; sessions: SessionSummary[]; vapidPublicKey?: string; agents?: ('claude' \| 'pi' \| 'opencode')[]; version?: string; home?: string }` (`agents`: the harness binaries the box found at start; absent means claude only. `version`: the daemon's app version, ADR 0021; absent from daemons before this shipped. `home`: the box's home directory, for showing a path as `~/…`; absent from daemons before this shipped)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `sessions.create`     | `{ repo; branch; base?: string; harness; title?; agent?: string; model?: string; effort?: string }` | `SessionSummary` (`harness` is the runtime `claude`/`pi`/`opencode`; `agent` is the name of a saved Agent, ADR 0023 § 2, whose model/effort/role seed the session — `bad_params` for an unknown name; `model`/`effort` are loose non-empty strings the box compiles to harness flags, ADR 0023, and override the Agent's; all omitted to spawn on the box defaults. The resolved `role` is persisted on the box and appended to the system prompt, not returned on the wire)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `sessions.createHelp` | `{ question: string }`                                                                              | `SessionSummary` (opens a daemon-managed **Help** session, ADR 0008: the daemon creates a read-only session on its own throwaway help repo under `<dataDir>/help` — never a repo under `reposDir` — with the shared Help role and read-only tools applied inline, then delivers `question` as the first `msg.user` through the same path `agent.send` uses. `question` is trimmed and `bad_params` when blank. Same result shape as `sessions.create`.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `sessions.restart`    | `{ session; model?: string \| null; effort?: string \| null }`                                      | `{}` (settles the agent's open asks as aborted and closes it; the next `agent.send` respawns it resuming its context. `model`/`effort` (ADR 0032) replace the session's configured ones for this and every later spawn: a string sets one, `null` clears it to the box's default, absent keeps it; loose free-text like `sessions.create`, trimmed, a blank one `bad_params`, so what is typed is what the harness flag gets)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `agent.send`          | `{ session; text; commentIds?: string[]; replyTo?: number; attachments?: string[] }`                | `{ seq: number }` (`attachments` are ids from `attach.end`, each complete and the session's own, else `bad_params`; `too_large` when they total over 50 MiB; `conflict` when the session closed while the message was being prepared. A `text` that is `/clear` alone, whitespace trimmed, is `sessions.clear` (ADR 0034): no `msg.user` is logged, the agent's context goes, and `seq` is the `session.cleared` marker's; `commentIds` are left pending for the next message, and with `replyTo` or `attachments` it is `bad_params`, since there is no message for them to ride on)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `settings.set`        | `{ flux?: Partial<FluxSettings>; harnessConfig?: Partial<HarnessConfig>; agents?: AgentSpec[] }`    | `Settings` (the whole state after the patch; `bad_params` and nothing written if any part is invalid, including an unknown key, a blank Agent name, or two Agents with the same name; `agents` replaces the whole saved-Agents list; `agent_unavailable` when `flux.defaultHarness` names a harness the box did not find, see `hello.agents`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `skills.list`         | `{}`                                                                                                | `{ skills: Skill[] }` (every `<name>/SKILL.md` under the flux user's `~/.claude/skills`, by name; a dir without a readable `SKILL.md` is skipped, a missing skills dir is an empty list. A daemon too old to have the method answers `not_found`, which the device reads as "no skills" — no editor, no slash autocomplete — never a hard failure)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `skills.write`        | `{ name; body }`                                                                                    | `{}` (creates or overwrites `~/.claude/skills/<name>/SKILL.md`, making the `<name>/` dir as needed. `name` is a safe single path segment — `bad_params` for the empty string, `.`/`..`, a `/` or `\`, an absolute path or a control character, so nothing is ever written outside the skills dir)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `skills.delete`       | `{ name }`                                                                                          | `{}` (removes the skill's `<name>/` directory; a name that is not there is a no-op. `name` is validated as for `skills.write`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `daemon.update`       | `{ version: string }`                                                                               | `{}` (self-update, ADR 0022: the daemon fetches that GitHub release, verifies its signature, atomically swaps its files and exits for the supervisor to restart. Returns at once; progress and failure come as the `update.progress` / `update.failed` ephemerals (§ 6), success as the reconnect's new `hello.version`. `unsupported` when the target is not valid semver, not newer than the running build, below the 1.0.0 floor, or the daemon runs from source)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `daemon.checkUpdate`  | `{}`                                                                                                | `{ current: string; latest: string \| null; available: boolean; verified: boolean \| null; reason?: string }` (update discovery + a verify-only dry-run, ADR 0021/0022. `current` is the running build; `latest` is the newest **published** release version — the box queries `GET https://api.github.com/repos/<repo>/releases/latest` and reads `tag_name`, which already excludes drafts and prereleases — or `null` (offline, no release, API error); `available` is whether a newer, floor-satisfying release could be installed (false on a source build, below the 1.0.0 floor, up to date, or `latest === null`); `verified` is the fetch-and-verify dry-run of that available release (`true`/`false`) or `null` when none ran; `reason` (non-empty when set) names the not-available or not-verified cause: `source_build` \| `below_floor` \| `up_to_date` \| `unreachable` \| `bad_signature` \| `bad_hash` \| `malformed`. The dry-run NEVER applies, NEVER writes the install dir and NEVER runs the fetched bytes. A daemon too old to have this method answers `not_found`; the device treats that (and `unsupported`) as "cannot check" and degrades) |
...
| `shell.run`           | `{ command: string; cwd?: string }`                                                                 | `{ runId }` (operator command runner, ADR 0026: spawns `sh -c <command>` with `cwd` defaulting to the repos dir — an optional `cwd` must resolve **inside** it (ADR-0018 `inside`), else `bad_params` — in a copy of the daemon env with every `FLUX_*` variable scrubbed plus `FORCE_COLOR=1`, and returns a `runId` at once. Output streams as the session-less `shell.output` / `shell.exited` ephemerals (§ 6), never logged, bounded to 256 KiB and 10 minutes. `conflict` when this device already has an active run — one at a time. Reachable only by a paired device; **no `flux_*` MCP tool exposes it and no agent can invoke it**.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `shell.interrupt`     | `{ runId }`                                                                                         | `{}` (escalates SIGINT→SIGTERM→SIGKILL on that run within a bounded budget, ADR 0017/0026. `not_found` for an unknown run or one belonging to another device, so a device can never see or kill another's run. A device disconnect or unpair kills its own active run — no orphaned processes.)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
interface SessionSummary {
  ...
  title: string;
  repo: string;
  branch: string; // the branch the session was created on, the one archive can delete
  head?: string; // where the worktree's HEAD is once the box has seen it move off `branch` (`session.head`), a detached HEAD as its short sha; absent until then, and from daemons before this shipped
  worktree?: string; // the worktree's absolute path on the box; absent from daemons before this shipped
  harness: 'claude' | 'pi' | 'opencode';
  model?: string; // the model the session spawns with: configured at create (ADR 0023) or set by a restart since (ADR 0032); absent on the box's default. Distinct from the running model, the log's latest `agent.spec` (§ 5)
  effort?: string; // the effort the session spawns with: configured at create (ADR 0023), set by a restart (ADR 0032) or in-band (ADR 0031) since; absent on the box's default
  state: 'idle' | 'running' | 'waiting_user' | 'ended';
  lastSeq: number;
...
interface FluxSettings {
  ...
  defaultHarness: 'claude' | 'pi' | 'opencode'; // must be in hello.agents; settings.set answers agent_unavailable otherwise
...
// The harness's global config files on the box, verbatim: ~/.claude/CLAUDE.md and
// ~/.claude/settings.json of the user the daemon runs as (ADR 0023). Empty string when a file is
// absent. `settingsJson` must be a JSON object on `settings.set`.
interface HarnessConfig {
  claudeMd: string;
  settingsJson: string;
}

// A box-side skill (`skills.*`): a `<name>/SKILL.md` file under the flux user's `~/.claude/skills`
// (Claude's skills convention). `name` is the skill's directory name, a safe single path segment;
// `body` is that file's contents. Flux only reads and writes these files — it is not an execution
// surface. Validated on `skills.write`/`skills.delete` so a name can never escape the skills dir.
interface Skill {
  name: string;
  body: string;
}
...
Error codes: `bad_params`, `not_found`, `not_paired`, `agent_unavailable`, `git_error`, `gh_error`, `conflict`, `dirty`, `too_large`, `unsupported`, `internal`. `unsupported` is returned by `daemon.update` for an update target the daemon refuses outright (ADR 0022): not valid semver, not newer than the running build, below the 1.0.0 floor, or a daemon running from source. `too_large` is returned by `attach.begin` for a file over 20 MiB and by `agent.send` for attachments totalling over 50 MiB. `conflict` is returned by `agent.send` for a message the session closed under while it was being prepared (an archive or restart from another device), by `fs.write` when `ifMatch` does not match the current file, and by `shell.run` (ADR 0026) when the calling device already has a command running. `dirty` is returned by `sessions.archive` with `removeWorktree` when the worktree holds uncommitted files or unpushed commits (no upstream: commits since the session's base) and `discard` is not set; the message says how many of each, for the device to show before asking again with `discard`.
...
The `sessions.createHelp` method (the "Ask about Flux" entry point, ADR 0008) is additive in the same way — one new method whose result reuses the existing `SessionSummary`, no existing shape changed — so `protocol` stays 2. It adds no event type: a Help session reuses the normal session machinery (`session.created`, `msg.user`, …) on a daemon-managed throwaway repo. A PWA served after it ships may hit an older self-updated daemon that lacks it; that daemon answers `not_found`, which the device can degrade (hide the entry point), never a hard failure.

The `shell.run` / `shell.interrupt` methods and the `shell.output` / `shell.exited` ephemerals (the operator command runner, ADR 0026) are additive in the same way — two new methods and two new session-less ephemerals, no existing shape changed — so `protocol` stays 2. They add no event type: command output is ephemeral by design and is never logged. A PWA served after they ship may hit an older self-updated daemon that lacks them; that daemon answers `not_found`, which the device degrades (it hides the command-runner entry point), never a hard failure.
